...
// PostgresInstances fetches PostgreSQL instances from the provider and returns them as resources.
func Instances(ctx context.Context, client providers.ProviderClient) ([]models.Resource, error) {
	resources := make([]models.Resource, 0)

	// Fetch PostgreSQL databases from the Linode provider
	databases, err := client.LinodeClient.ListPostgresDatabases(ctx, nil)
	if err != nil {
	...
	for _, database := range databases {
		...
		if err != nil {
			log.Warnf("Failed to get cluster size for PostgreSQL database: %d, Error: %s", database.ID, err.Error())
		...
		if !ok {
			log.Warnf("Failed to calculate cost for PostgreSQL database: %d, Type: %s", database.ID, database.Type)
			// Skip this database and continue with the next one
			continue
		}
...
// GetClusterSize retrieves the cluster size for a specific PostgreSQL instance.
func GetClusterSize(ctx context.Context, client providers.ProviderClient, instanceID int) (int, error) {
	instance, err := client.LinodeClient.GetPostgresDatabase(ctx, instanceID)
	if err != nil {
		return 0, err
	}
...
// InstancesCost calculates the cost for the given PostgreSQL instance type and cluster size.
