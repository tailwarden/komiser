	...
	for {
		...
		for _, disk := range disksListPair.Value.Disks {
			...
			resources = append(resources, models.Resource{
				...
				FetchedAt:  time.Now(),
				Tags:       tags,
				Link:       fmt.Sprintf("https://console.cloud.google.com/compute/instancesDetail/zones/%s/instances/%s?project=%s", zone, disk.GetName(), client.GCPClient.Credentials.ProjectID),
			})
		}
	}
