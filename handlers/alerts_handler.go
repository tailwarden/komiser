...
func (handler *ApiHandler) NewAlertHandler(c *gin.Context) {
	...
	result, err := handler.repo.HandleQuery(c, "INSERT", &alert, nil)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (handler *ApiHandler) UpdateAlertHandler(c *gin.Context) {
	...
	_, err = handler.repo.HandleQuery(c, "UPDATE_ALERT", &alert, [][3]string{{"id", "=", fmt.Sprint(alertId)}})
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (handler *ApiHandler) DeleteAlertHandler(c *gin.Context) {
	...
	_, err := handler.repo.HandleQuery(c, "DELETE", alert, [][3]string{{"id", "=", fmt.Sprint(alertId)}})
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
