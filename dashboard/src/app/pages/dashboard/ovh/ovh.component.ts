...
export class OvhDashboardComponent implements OnInit, AfterViewInit {
    ...
    constructor(private ovhService: OvhService) {
        this.ovhService.getCloudProjects().subscribe(
            (data) => {
        ...
        this.ovhService.getUsers().subscribe(
            (data) => {
        ...
        this.ovhService.getCurrentBill().subscribe(
            (data) => {
        ...
        this.ovhService.getCloudInstances().subscribe(
            (data) => {
                let plots = {};
                let _usedRegions = new Map<string, number>();

                data.forEach((instance) => {
                    _usedRegions[instance.region] =
                        (_usedRegions[instance.region]
                            ? _usedRegions[instance.region]
                            : 0) + 1;
                });
        ...
        this.ovhService.getTickets().subscribe(
            (data) => {
