...
export class GcpDashboardComponent implements OnInit, AfterViewInit {
    ...
    constructor(
        ...
        this.gcpService.getComputeInstances().subscribe(
            (data) => {
                ...
                data.forEach((instance) => {
                    let region = instance.zone.substring(
                        0,
                        instance.zone.lastIndexOf('-')
                    );
                    _usedRegions[region] =
                ...
                for (var region in _usedRegions) {
                    ...
                    plots[region] = {
                        ...
                        tooltip: {
                            content: `${region}<br />Instances: ${_usedRegions[region]}`,
                        },
                    };
                }
        ...
        this.gcpService.getIAMUsers().subscribe(
            (data) => {
        ...
        this.gcpService.getBillingPerService().subscribe(
            (data) => {
                ...
                data.forEach((period) => {
                    periods.push(
                        new Date(period.date).toLocaleString('en-us', {
                            month: 'long',
                        })
                    );
                });
        ...
        this.gcpService.getBillingLastSixMonths().subscribe(
            (data) => {
