...
export class AwsComputeComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    private initState() {
        ...
        this.awsService.getDetachedElasticIps().subscribe(
            (data: any) => {
        ...
        this.awsService.getInstancesPerRegion().subscribe(
            (data) => {
                this.runningEC2Instances = data.state.running
                    ? data.state.running
                    : 0;
                this.stoppedEC2Instances = data.state.stopped
                    ? data.state.stopped
                    : 0;
                this.terminatedEC2Instances = data.state.terminated
        ...
        this.awsService.getLambdaFunctions().subscribe(
            (data: any) => {
        ...
        this.awsService.getLambdaInvocationMetrics().subscribe(
            (data) => {
                let labels = [];
                data.forEach((period) => {
                    labels.push(
                        new Date(period.timestamp).toLocaleString('en-us', {
                            month: 'long',
                        })
                    );
                });
        ...
        this.awsService.getECS().subscribe(
            (data) => {
        ...
        this.awsService.getLambdaErrors().subscribe(
            (data) => {
                let labels = [];
                data.forEach((period) => {
                    labels.push(
                        new Date(period.timestamp).toISOString().split('T')[0]
                    );
                });
        ...
        this.awsService.getReservedInstances().subscribe(
            (data) => {
        ...
        this.awsService.getScheduledInstances().subscribe(
            (data) => {
        ...
        this.awsService.getSpotInstances().subscribe(
            (data) => {
        ...
        this.awsService.getEKSClusters().subscribe(
            (data) => {
        ...
        this.awsService.getCostPerInstanceType().subscribe(
            (data) => {
                let periods = [];
                let series = [];
    ...
    private showCostPerInstanceType(labels, series) {
        ...
        this.costPerInstanceTypeChart = new Chartist.Bar(
            '.costPerInstanceTypeChart',
            costHistory,
    ...
    private showLambdaErrors(labels, series) {
        let scope = this;
        this.lambdaErrorsChart = new Chartist.Line(
            '.lambdaErrorsChart',
            {
                labels: labels,
                series: series,
    ...
    private showLambdaInvocations(labels, series) {
        let scope = this;
        this.lambdaInvocationsChart = new Chartist.Bar(
            '.lambdaInvocationsChart',
            {
                labels: labels,
                series: series,
            },
