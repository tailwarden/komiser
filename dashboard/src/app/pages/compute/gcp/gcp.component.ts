...
export class GcpComputeComponent implements OnInit, AfterViewInit {
    ...
    private zones: Map<string, any> = new Map<string, any>([
        ...
        [
            ...
            { latitude: '43.498299', longitude: '2.375200' },
        ],
        ...
        [
            ...
            { latitude: '45.509060', longitude: '-73.553360' },
        ],
        [
            'southamerica-east1',
            { latitude: '23.550651', longitude: '-46.633382' },
        ],
    ...
    constructor(private gcpService: GcpService) {
        ...
        this.gcpService.getComputeInstances().subscribe(
            (data) => {
                ...
                data.forEach((instance) => {
                    ...
                    machineTypes[instance.machineType] =
                        (machineTypes[instance.machineType]
                            ? machineTypes[instance.machineType]
                            : 0) + 1;
                });
        ...
        this.gcpService.getCloudFunctions().subscribe(
            (data) => {
        ...
        this.gcpService.getKubernetesClusters().subscribe(
            (data) => {
                ...
                data.forEach((cluster) => {
                    let region = cluster.zone.substring(
                        0,
                        cluster.zone.lastIndexOf('-')
                    );
                    _usedRegions[region] =
        ...
        this.gcpService.getComputeCPUUtilization().subscribe(
            (data) => {
                data.forEach((metric) => {
                    let series = [];
                ...
                this.returnedInstancesWithCPUUtilization =
                    this.instancesWithCPUUtilization.slice(0, 3);
        ...
        this.gcpService.getAppEngineBandwidth().subscribe(
            (data) => {
    ...
    pageChanged(event: PageChangedEvent): void {
        ...
        this.returnedInstancesWithCPUUtilization =
            this.instancesWithCPUUtilization.slice(startItem, endItem);
    }
