...
export class OvhStorageComponent implements OnInit {
    ...
    constructor(private ovhService: OvhService) {
        this.ovhService.getStorageContainers().subscribe(
            (data) => {
                let totalObjects = 0;
                let totalSize = 0;
                ...
                this.storageContainerTotalSize =
                    this.bytesToSizeWithUnit(totalSize);
        ...
        this.ovhService.getCloudVolumes().subscribe(
            (data) => {
                ...
                this.volumesSize = this.bytesToSizeWithUnit(
        ...
        this.ovhService.getCloudSnapshots().subscribe(
            (data) => {
                let totalSize = 0;
                data.forEach((snapshot) => {
                    totalSize += snapshot.size;
                });
                this.snapshots = this.bytesToSizeWithUnit(
                    totalSize * 1024 * 1024 * 1024
                );
