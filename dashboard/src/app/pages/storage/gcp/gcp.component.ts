...
export class GcpStorageComponent implements OnInit {
    ...
    constructor(private gcpService: GcpService) {
        this.gcpService.getStorageBuckets().subscribe(
            (data) => {
        ...
        this.gcpService.getComputeDisks().subscribe(
            (data) => {
                let total = 0;
                this.disksTotal = 0;
                data.forEach((disk) => {
                ...
                this.disksTotalSize = this.bytesToSizeWithUnit(
        ...
        this.gcpService.getSqlInstances().subscribe(
            (data) => {
        ...
        this.gcpService.getDiskSnapshots().subscribe(
            (data) => {
                ...
                this.snapshotsSize = this.bytesToSizeWithUnit(
                    total * 1024 * 1024 * 1024
                );
            },
            (err) => {
        ...
        this.gcpService.getBucketsSize().subscribe(
            (data) => {
                ...
                points.forEach((point) => {
                    labels.push(
                        new Date(point.interval.endTime)
                            .toISOString()
                            .split('T')[0]
                    );
                    series.push({
                ...
                this.storageBucketSize = this.bytesToSizeWithUnit(
        ...
        this.gcpService.getBucketsObjects().subscribe(
            (data) => {
                let points = data[0].points;
                let labels = [];
                let series = [];
                ...
                this.storageBucketObjects = this.formatNumber(
                    series[series.length - 1].value
                ).toString();
        ...
        this.gcpService.getIngestedLoggingBytes().subscribe(
            (data) => {
                let availablePeriods = [];
                data.forEach((resource) => {
                    resource.points.forEach((point) => {
                        let timestamp = new Date(point.interval.endTime)
                            .toISOString()
                            .split('T')[0];
        ...
        this.gcpService.getComputeImages().subscribe(
            (data) => {
                let total = 0;
                data.forEach((image) => {
                    total += image.size;
                ...
                this.imagesSize = this.bytesToSizeWithUnit(
                    total * 1024 * 1024 * 1024
                );
            },
            (err) => {
        ...
        this.gcpService.getRedisInstances().subscribe(
            (data) => {
