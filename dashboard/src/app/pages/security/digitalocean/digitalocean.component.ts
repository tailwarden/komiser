...
export class DigitaloceanSecurityComponent implements OnInit, OnDestroy {
    ...
    private initState() {
        ...
        this.digitaloceanService.getSshKeys().subscribe(
            (data) => {
                this.sshKeys = data;
        ...
        this.digitaloceanService.getListOfFirewalls().subscribe(
            (data) => {
                this.firewalls = data;
        ...
        this.digitaloceanService.getCertificates().subscribe(
            (data) => {
        ...
        this.digitaloceanService.getUnsecureFirewalls().subscribe(
            (data) => {
                this.unrestrictedFirewalls = data;
                this.returnedUnrestrictedFirewalls =
                    this.unrestrictedFirewalls.slice(0, 20);
            },
            (err) => {
                this.unrestrictedFirewalls = [];
                this.returnedUnrestrictedFirewalls = [];
        ...
        this.digitaloceanService.getActionsHistory().subscribe(
            (data) => {
                this.actions = data;
    ...
    pageChanged(event: PageChangedEvent): void {
        const startItem = (event.page - 1) * event.itemsPerPage;
        const endItem = event.page * event.itemsPerPage;
        this.returnedUnrestrictedFirewalls = this.unrestrictedFirewalls.slice(
            startItem,
            endItem
        );
    }
